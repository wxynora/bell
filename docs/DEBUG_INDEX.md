...
- 定向验证：`bash -n deploy/scripts/verify-systemd-crash-cleanup.sh`；`npm run check`（33/33、本地双 typecheck、构建）；目标 Linux 上的真实拒绝 SIGTERM 孙进程 crash 清理通过。生产 SSE 已连接，既有首只 wake 完成 accepted／ACK；最终启用前后 wake 与网关任务计数均保持 1。首户 injector 的隔离回归另要求一个 wake 只生成一个 system 消息，没有追加 user 消息或真实模型调用。
